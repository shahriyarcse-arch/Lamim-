...
if (!window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
  ...
  // On mobile, skip hero-art / tile / float scale+fade animations to prevent blinking
  ...
  if (!isMobile) {
    ...
    // Mobile: show hero card immediately without animation
    const heroArt = document.querySelector('.hero-art');
    if (heroArt) { heroArt.style.opacity = '1'; heroArt.style.transform = 'none'; }
    document.querySelectorAll('.hero-art .tile').forEach((el) => {
