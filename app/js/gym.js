const Gym = {
  ...
  exportPDF() {
    ...
    let totalCalories = 0, totalProtein = 0, mealDays = 0;
    for (let d = 1; d <= daysInMonth; d++) {
      const dateStr = year + '-' + String(month + 1).padStart(2, '0') + '-' + String(d).padStart(2, '0');
      const gData = DB.getGym(dateStr);
      if (gData && gData.diet && gData.diet.meals && gData.diet.meals.length > 0) {
        mealDays++;
        gData.diet.meals.forEach(m => {
    ...
    <style>
      ...
      * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
      ...
      .meta span { font-size: 7.5px; color: #99f6e4; font-weight: 700; }
      ...
      .sum-card::before { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 2.5px; background: var(--c, #0d9488); }
      ...
      .recovery-matrix-box { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 6px 10px; margin-bottom: 6px; display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; text-align: center; }
      .rm-lbl { font-size: 7px; font-weight: 800; color: #64748b; text-transform: uppercase; }
      .rm-val { font-size: 11.5px; font-weight: 900; color: #0f172a; margin-top: 1px; }

      .consistency-bar { display: flex; justify-content: space-between; align-items: center; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 4px 8px; margin-bottom: 6px; font-size: 7.5px; font-weight: 800; }
      .grid-tables { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 6px; }
      table { width: 100%; border-collapse: collapse; font-size: 8px; border: 1px solid #e2e8f0; border-radius: 6px; overflow: hidden; background: #fff; table-layout: fixed; }
      ...
      .ayah-quote { background: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 6px; padding: 6px 10px; margin-bottom: 6px; color: #065f46; font-size: 7.5px; line-height: 1.3; text-align: center; }
