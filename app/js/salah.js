...
const Salah = {
  ...
  prayerMeta: {
    ...
    dhuhr:   { label: 'Dhuhr',   icon: '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><g style="transform-origin: 12px 12px"><circle cx="12" cy="12" r="4"><animate attributeName="r" values="3.6; 4.4; 3.6" dur="3s" repeatCount="indefinite"/><animate attributeName="opacity" values="0.85; 1; 0.85" dur="2.4s" repeatCount="indefinite"/></circle><path d="M12 2v2"/><path d="M12 20v2"/><path d="m4.93 4.93 1.41 1.41"/><path d="m17.66 17.66 1.41 1.41"/><path d="M2 12h2"/><path d="M20 12h2"/><path d="m6.34 17.66-1.41 1.41"/><path d="m19.07 4.93-1.41 1.41"/></g></svg>', gradient: 'linear-gradient(135deg, #fbbf24, #f59e0b)', timeLabel: 'Midday', glow: '0 0 20px rgba(251,191,36,0.5)' },
    asr:     { label: 'Asr',     icon: '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><g><animateTransform attributeName="transform" type="translate" values="-1,0; 1,0; -1,0" dur="5s" repeatCount="indefinite"/><path d="M13 22H7a5 5 0 1 1 4.9-6H13a3 3 0 0 1 0 6Z"/></g><g style="transform-origin: 12px 12px"><path d="M12 2v2"/><path d="m4.93 4.93 1.41 1.41"/><path d="M20 12h2"/><path d="m19.07 4.93-1.41 1.41"/><path d="M15.947 12.65a4 4 0 0 0-5.925-4.128"/></g></svg>', gradient: 'linear-gradient(135deg, #fb923c, #f97316)', timeLabel: 'Afternoon', glow: '0 0 20px rgba(251,146,60,0.5)' },
  ...
  statusMeta: {
    ...
    qaza:   { label: 'Qaza',    icon: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><g style="transform-origin: 12px 12px"><circle cx="12" cy="12" r="9.5"><animate attributeName="stroke-width" values="1.8; 2.6; 1.8" dur="2.2s" repeatCount="indefinite"/><animate attributeName="opacity" values="0.7; 1; 0.7" dur="2.2s" repeatCount="indefinite"/></circle><line x1="12" y1="12" x2="12" y2="7"></line><g style="transform-origin: 12px 12px"><animateTransform attributeName="transform" type="rotate" values="0; 25; 0; -15; 0" dur="3s" repeatCount="indefinite"/><line x1="12" y1="12" x2="16.5" y2="13.5"></line></g><circle cx="12" cy="12" r="2" fill="currentColor"><animate attributeName="opacity" values="0.6; 1; 0.6" dur="1.5s" repeatCount="indefinite"/></circle></g></svg>', color: '#d29922', bgAlpha: 'rgba(210,153,34,0.12)', borderAlpha: 'rgba(210,153,34,0.4)', desc: 'Made up later', result: 'qaza',       points: 3, glow: '0 0 12px rgba(210,153,34,0.6)' },
  ...
  exportPDF() {
    ...
    const totalJamaatCount = Object.values(prayerStats).reduce((a, b) => a + b.jamaat, 0);
    const jamaatRatioPct = totalPrayed > 0 ? Math.round((totalJamaatCount / totalPrayed) * 100) : 0;
    ...
    const prayerPillsHtml = this.prayers.map(p => {
      ...
      return `<div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px; padding:6px 8px; display:flex; flex-direction:column; gap:4px;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          ...
          <span style="font-weight:800; color:#4f46e5; font-size:8.5px;">${pPct}%</span>
        </div>
  ...
  <div class="page">
    <div>
      ...
      <div class="summary-grid">
        ...
        <div class="summary-card">
          <div class="sum-val">${jamaatRatioPct}%</div>
          <div class="sum-lbl">Congregation (Jama'at) Rate</div>
        </div>
      </div>
      ...
      <div class="spiritual-audit-box">
        <div><div class="sab-item-lbl">Farz Completion</div><div class="sab-item-val" style="color:#059669;">${Math.round((totalPrayed / (daysInMonth * 5)) * 100)}%</div></div>
        <div><div class="sab-item-lbl">Jama'at Count</div><div class="sab-item-val" style="color:#10b981;">${totalJamaatCount}</div></div>
        <div><div class="sab-item-lbl">Days Tracked</div><div class="sab-item-val" style="color:#4f46e5;">${daysInMonth} Days</div></div>
        <div><div class="sab-item-lbl">Audit Status</div><div class="sab-item-val" style="color:#7c3aed;">Verified</div></div>
      </div>
    ...
    <div class="footer">
      ...
      <span class="footer-brand">v2.1.0 "Aura" • Page 1 of 1</span>
    </div>
  </div>
</body>
</html>`;
